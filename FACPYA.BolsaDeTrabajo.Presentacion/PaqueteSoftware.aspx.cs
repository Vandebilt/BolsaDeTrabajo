...
{
    public partial class PaqueteSoftware : System.Web.UI.Page
    {
        ...
        protected void LimpiarFormulario()
        {
            ...
            txtPaqueteSoftware.Text = string.Empty;
            txtBusquedaPaqueteSoftware.Text = string.Empty;
            vpId = 0;
        }
        #endregion
        ...
        // Cargar GridView
        protected List<DataRow> CargarGridView(int? pIdEstatus, string pPaqueteSoftware)
        {
            // Accesa a la función para llenar los datos y activa el Grid para la paginación
            var lista = logicaPaqueteSoftware.Consultar(pIdEstatus, pPaqueteSoftware);
        ...
        protected void gvConsultaGeneral_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "Seleccionar" || e.CommandName == "Eliminar" || e.CommandName == "Suspender")
            {
                ...
                List<bdPaqueteSoftware> lista = logicaPaqueteSoftware.Leer(pId);

                if (e.CommandName == "Seleccionar")
                {
                    vpId = pId;
                    txtPaqueteSoftware.Text = lista[0].PaqueteSoftware.ToString();
                }
                else if (e.CommandName == "Eliminar")
                {
                    // Guarda el ID de la Habilidad y muestra un mensaje de confirmación
                    Session["_Id"] = pId;
                    ClientScript.RegisterStartupScript(this.GetType(), "Mensaje", $"mostrarConfirmacion('Confirmación de solicitud','¿Estás seguro de eliminar el registro seleccionado?','PaqueteSoftware','confirmarEliminacion','')", true);
                }
                else if (e.CommandName == "Suspender")
                {
                    ...
                    if (lista[0].IdEstatus == 1)
                    {
                        // Guarda el ID de la Habilidad y muestra un mensaje de confirmación
                        Session["_Id"] = pId;
                        ClientScript.RegisterStartupScript(this.GetType(), "Mensaje", $"mostrarConfirmacion('Confirmación de solicitud','¿Estás seguro de suspender el registro seleccionado?','PaqueteSoftware','confirmarSuspender','')", true);
                    }
                    ...
                    else if (lista[0].IdEstatus == 7)
                    {
                        // Guarda el ID de la Habilidad y muestra un mensaje de confirmación
                        Session["_Id"] = pId;
                        ClientScript.RegisterStartupScript(this.GetType(), "Mensaje", $"mostrarConfirmacion('Confirmación de solicitud','¿Estás seguro de reactivar el registro seleccionado?','PaqueteSoftware','confirmarSuspender','')", true);
                    }
                }
            }
        }
        ...
        public void FuncionDeshabilitarControles(GridView pGridView)
        {
            foreach (GridViewRow row in pGridView.Rows)
            {
                ...
                // Obtener el valor de la celda en la columna 6 para determinar si el botón debe estar habilitado
                string valorCelda = "";
        ...
        protected void btnGrabar_Click(object sender, EventArgs e)
        {
            ...
            {
                { "vPaqueteSoftware", txtPaqueteSoftware.Text },
            };

            List<string> listaValidacion = validacionPaqueteSoftware.Validar(diccionario);

            if (listaValidacion.Count == 0)
            {
                bdPaqueteSoftware parametro = new bdPaqueteSoftware
                {
                    IdPaqueteSoftware = vpId,
                    Descripcion = txtPaqueteSoftware.Text.ToUpper(),
                ...
                {
                    // Crear
                    control = logicaPaqueteSoftware.Crear(parametro);
                }
                else
                {
                    // Editar
                    control = logicaPaqueteSoftware.Editar(parametro);
                }
        ...
        protected void btnBuscar_Click(object sender, EventArgs e)
        {
            ...
            string pPaqueteSoftware = string.IsNullOrEmpty(txtPaqueteSoftware.Text.Trim()) ? null : txtPaqueteSoftware.Text.Trim();

            // Crea una variable de tipo sesión para mantener el filtro dentro de la paginación
            var lista = CargarGridView(pIdEstatus, pPaqueteSoftware);
        ...
        {
            // Página de Idioma
            PaqueteSoftware paqueteSoftware = new PaqueteSoftware();

            // Verifica si la confirmación es válida
            if (pConfirmacion.Equals("Confirmación"))
            {
                // Obtiene el ID desde la sesión
                int pIdPaqueteSoftware = (int)paqueteSoftware.Session["_Id"];
                var lista = logicaPaqueteSoftware.Leer(pIdPaqueteSoftware);

                if (lista.Count > 0)
                {
                    bdPaqueteSoftware parametro = new bdPaqueteSoftware
                    {
                        IdPaqueteSoftware = pIdPaqueteSoftware,
                        UsuarioBaja = paqueteSoftware.IdUsuarioSesion()
                    };

                    // Eliminar proveedor de la base de datos
                    var control = logicaPaqueteSoftware.Eliminar(parametro);
                    paqueteSoftware.MostrarMensaje(control.Item1, control.Item2, null);
        ...
        public static string confirmarSuspender(string pConfirmacion)
        {
            ...
            if (pConfirmacion.Equals("Confirmación"))
            {
                ...
                if (lista.Count > 0)
                {
                    ...
                    {
                        IdPaqueteSoftware = pIdPaqueteSoftware,
                        UsuarioModificacion = paqueteSoftware.IdUsuarioSesion()
                    };
                    ...
                    {
                        // SUSPENDE
                        control = logicaPaqueteSoftware.Suspender(parametro);
                    }
                    else if (lista[0].IdEstatus == 7)
                    {
                        // ACTIVA
                        control = logicaPaqueteSoftware.Activar(parametro);
                    }
        ...
        protected void btnExportar_Click(object sender, EventArgs e)
        {
            ...
            DataTable dataTable = logicaPaqueteSoftware.ExportarExcel(pIdEstatus, pPaqueteSoftware);
            dataTable.TableName = "REPORTE PAQUETE DE SOFTWARE"; // Establece el nombre de la pestaña en el libro

            // Nombre del archivo a descargar
            logicaExportarExcel.ExportarMicrosoftExcel(dataTable, "REPORTE PAQUETE DE SOFTWARE" + DateTime.Now.ToString("dd-MM-yyyy"));
        }
        #endregion
    }
}