...
{
    public partial class Habilidad : System.Web.UI.Page
    {
        ...
        protected void LimpiarFormulario()
        {
            ...
            ddlBusquedaIdTipoHabilidad.ClearSelection();
            txtBusquedaHabilidad.Text = string.Empty;
            vpId = 0;
        }
        #endregion
        ...
        // Cargar GridView
        protected List<DataRow> CargarGridView(int? pIdEstatus, int? pIdTipoHabilidad, string pHabilidad)
        {
            // Accesa a la función para llenar los datos y activa el Grid para la paginación
            var lista = logicaHabilidad.Consultar(pIdEstatus, pIdTipoHabilidad, pHabilidad);
        ...
        protected void CargarDropDownLists()
        {
            ...
            master.CargarDropDownList(ddlBusquedaIdTipoHabilidad, 20, null);
        ...
        protected void gvConsultaGeneral_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "Seleccionar" || e.CommandName == "Eliminar" || e.CommandName == "Suspender")
            {
                ...
                List<bdHabilidad> lista = logicaHabilidad.Leer(pId);

                if (e.CommandName == "Seleccionar")
                {
                    vpId = pId;
                    ddlIdTipoHabilidad.SelectedValue = lista[0].IdTipoHabilidad.ToString();
                    txtHabilidad.Text = lista[0].Habilidad.ToString();
                }
                else if (e.CommandName == "Eliminar")
                {
                    // Guarda el ID de la Habilidad y muestra un mensaje de confirmación
                    Session["_Id"] = pId;
                    ClientScript.RegisterStartupScript(this.GetType(), "Mensaje", $"mostrarConfirmacion('Confirmación de solicitud','¿Estás seguro de eliminar el registro seleccionado?','Habilidad','confirmarEliminacion','')", true);
                }
                else if (e.CommandName == "Suspender")
                {
                    ...
                    if (lista[0].IdEstatus == 1)
                    {
                        // Guarda el ID de la Habilidad y muestra un mensaje de confirmación
                        Session["_Id"] = pId;
                        ClientScript.RegisterStartupScript(this.GetType(), "Mensaje", $"mostrarConfirmacion('Confirmación de solicitud','¿Estás seguro de suspender el registro seleccionado?','Habilidad','confirmarSuspender','')", true);
                    }
                    ...
                    else if (lista[0].IdEstatus == 7)
                    {
                        // Guarda el ID de la Habilidad y muestra un mensaje de confirmación
                        Session["_Id"] = pId;
                        ClientScript.RegisterStartupScript(this.GetType(), "Mensaje", $"mostrarConfirmacion('Confirmación de solicitud','¿Estás seguro de reactivar el registro seleccionado?','Habilidad','confirmarSuspender','')", true);
                    }
                }
            }
        }
        ...
        protected void btnGrabar_Click(object sender, EventArgs e)
        {
            ...
            Dictionary<object, string> diccionario = new Dictionary<object, string>
            {
                ...
                { "vHabilidad", txtHabilidad.Text },
            };

            List<string> listaValidacion = validacionHabilidad.Validar(diccionario);

            if (listaValidacion.Count == 0)
            {
                bdHabilidad parametro = new bdHabilidad
                {
                    IdHabilidad = vpId,
                    IdTipoHabilidad = Convert.ToInt32(ddlIdTipoHabilidad.SelectedValue),
                    Descripcion = txtHabilidad.Text.ToUpper(),
                ...
                {
                    // Crear
                    control = logicaHabilidad.Crear(parametro);
                }
                else
                {
                    // Editar
                    control = logicaHabilidad.Editar(parametro);
                }
        ...
        protected void btnBuscar_Click(object sender, EventArgs e)
        {
            ...
            int? pIdTipoHabilidad = int.TryParse(ddlBusquedaIdTipoHabilidad.SelectedValue, out int tip) && tip != 0 ? (int?)tip : null;
            string pHabilidad = string.IsNullOrEmpty(txtBusquedaHabilidad.Text.Trim()) ? null : txtBusquedaHabilidad.Text.Trim();

            // Crea una variable de tipo sesión para mantener el filtro dentro de la paginación
            var lista = CargarGridView(pIdEstatus, pIdTipoHabilidad, pHabilidad);
        ...
        public static string confirmarEliminacion(string pConfirmacion)
        {
            ...
            if (pConfirmacion.Equals("Confirmación"))
            {
                ...
                int pIdHabilidad = (int)habilidad.Session["_Id"];
                var lista = logicaHabilidad.Leer(pIdHabilidad);

                if (lista.Count > 0)
                {
                    bdHabilidad parametro = new bdHabilidad
                    {
                        IdHabilidad = pIdHabilidad,
                        UsuarioBaja = habilidad.IdUsuarioSesion()
                    };

                    // Eliminar de la base de datos
                    var control = logicaHabilidad.Eliminar(parametro);
                    habilidad.MostrarMensaje(control.Item1, control.Item2, null);
        ...
        public static string confirmarSuspender(string pConfirmacion)
        {
            ...
            if (pConfirmacion.Equals("Confirmación"))
            {
                ...
                if (lista.Count > 0)
                {
                    ...
                    {
                        IdHabilidad = pIdHabilidad,
                        UsuarioModificacion = habilidad.IdUsuarioSesion()
                    };
                    ...
                    {
                        // SUSPENDE
                        control = logicaHabilidad.Suspender(parametro);
                    }
                    else if (lista[0].IdEstatus == 7)
                    {
                        // ACTIVA
                        control = logicaHabilidad.Activar(parametro);
                    }
        ...
        protected void btnExportar_Click(object sender, EventArgs e)
        {
            ...
            DataTable dataTable = logicaHabilidad.ExportarExcel(pIdEstatus, pIdTipoHabilidad, pHabilidad);
            dataTable.TableName = "REPORTE HABILIDAD"; // Establece el nombre de la pestaña en el libro

            // Nombre del archivo a descargar
            logicaExportarExcel.ExportarMicrosoftExcel(dataTable, "REPORTE HABILIDAD " + DateTime.Now.ToString("dd-MM-yyyy"));
        }
        #endregion
    }
}